...
namespace Match.ParsingCode
{
    public static class XBetParser
    {
        ...
        public static List<SoccerMatch> GetData()
        {
        ...
        private static void FillCoefs(SoccerMatch match, HtmlNode[] betList)
        {
            ...
            coef_value = HapHelper.GetChildNodeByAttribute(bet, "class", "num").InnerText.Replace(" ", string.Empty).Replace(".", ",").Substring(1);
            match.Line.Total = Convert.ToDouble(coef_value);
        ...
        {
            int t = (int)type;
            string coef = HapHelper.GetChildNodeByAttribute(bet, "a", "data-type", t.ToString())
                 .InnerText
